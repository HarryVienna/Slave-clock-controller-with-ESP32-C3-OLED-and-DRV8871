...
esp_err_t WifiProvisioner::connect_sta() {
    ...
    ESP_LOGI(TAG, "WiFi system started (TX power set to 20 dBm). Waiting for connection events...");
...
esp_err_t WifiProvisioner::scan_get_handler_(httpd_req_t *req) {
    ...
    // 2. Liste nach Signalstärke sortieren (stärkste zuerst)
    std::sort(ap_records.begin(), ap_records.end(), [](const wifi_ap_record_t& a, const wifi_ap_record_t& b) {
        return a.rssi > b.rssi;
    });

    // 3. JSON erstellen - dabei leere und doppelte SSIDs filtern
